 ...
 *
 *  - Collapsing the control card's tab body lets the subtitle output fill the
...
/* Everything the background click must keep its hands off: the card itself,
   the manual overlay floating over the output, any open dialog (a click on a
...
/* .tour-root earns its place for the tour's own interactive step: the blocker
   steps aside there so a real background click can collapse the panel, which
   would otherwise make the tour card itself a collapse button. */
const FOREGROUND = '.control-card, .manual-translate-panel, dialog, .color-popover, .tour-root';
...
function initPanelCollapse() {
  ...
  /* Where the press started matters as much as where it ended: a drag that
     begins on a slider inside the card and finishes over the output pane is not a
...
/* Collapsed mode hides the tab body, and the language routing goes with it.
   ...
   language?" is still answerable while the output is being captured. It is
